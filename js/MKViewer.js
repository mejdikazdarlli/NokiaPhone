...
export class MKviewer {
   ...
   async initScene() {
        ...
        try {
            await Promise.all([
                LoadModel('3310.glb', this.scene, this.renderer),
                LoadWeapon('M4A4.glb', this.scene, this.camera, 0.3, 0, -0.1, -1),
                loadEnvironmentHDR(this.renderer, this.scene, 'metal_03.hdr')
            ]);
            console.log('All models and environment HDR are loaded. Performing actions...');
            // Do something...
    
        } catch (error) {
            console.error('Error loading models or environment HDR:', error);
        }
...
export async function LoadWeapon(model,_thisScene, _thisCamera,vecScale,vecPositionX,vecPositionY,vecPositionZ) {
    return new Promise((resolve, reject) => {
        ...
        async function (object) {
            ...
            clips.forEach(clip => {action = mixer.clipAction(clip)});
            LoadedModel = true
        resolve()
